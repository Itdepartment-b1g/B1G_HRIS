...
export type QuickFilterAndClause<T extends string> = {
  id: string;
  field: T | 'all';
  value: string;
};
...
type QuickFilterSheetProps<TColumn extends string, TStatus extends string> = {
  ...
  onColumnClausesChange: (clauses: QuickFilterAndClause<TColumn>[]) => void;
  status: TStatus;
...
export function QuickFilterSheet<TColumn extends string, TStatus extends string>({
  ...
  const updateClause = (id: string, patch: Partial<QuickFilterAndClause<TColumn>>) => {
    onColumnClausesChange(rows.map((clause) => (clause.id === id ? { ...clause, ...patch } : clause)));
  };
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent
        ...
        className="z-[70] flex w-full flex-col gap-0 p-0 sm:max-w-md"
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-4 py-3">
          <FilterSection title="Date">
            <DateRangeFilterPopover
              ...
              className="w-[360px] max-w-[calc(100vw-2rem)] p-0 z-[80]"
