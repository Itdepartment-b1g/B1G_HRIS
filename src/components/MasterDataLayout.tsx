...
import { Users, Network, Clock, Settings, ChevronRight } from 'lucide-react';
import { cn } from '@/lib/utils';

const NAV_ITEMS = [
  {
    path: '/dashboard/master-data/employees',
    label: 'Employee List',
    icon: Users,
    description: 'Manage all employees',
  },
  {
    path: '/dashboard/master-data/departments',
    label: 'Department',
    icon: Network,
    description: 'Department management',
  },
  {
    path: '/dashboard/master-data/shifts',
    label: 'Shift',
    icon: Clock,
  ...
  return (
    <div className="flex gap-6 min-h-[calc(100vh-120px)]">
      ...
      <div className="lg:hidden w-full">
        <div className="flex gap-2 mb-4 border-b pb-3 overflow-x-auto">
          {NAV_ITEMS.map((item) => {
            const Icon = item.icon;
            const active = location.pathname === item.path;
            return (
