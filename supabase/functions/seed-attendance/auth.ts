...
export interface AuthResult { userId: string; roles: string[] }
...
export function requireSuperAdmin(auth: AuthResult | null, req: Request): Response | null {
  ...
  if (!auth) return new Response(JSON.stringify({ error: 'Unauthorized' }), { headers, status: 401 })
  if (!auth.roles.includes('super_admin')) return new Response(JSON.stringify({ error: 'Forbidden: super_admin role required' }), { headers, status: 403 })
  return null
}
