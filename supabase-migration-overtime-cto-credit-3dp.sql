...
BEGIN
  ...
  -- On approval: credit OT hours to CTO leave balance first (1h = 0.125 day)
  ...
  IF p_action = 'approved' AND COALESCE(v_rec.hours, 0) > 0 THEN
    ...
    IF v_has_cto THEN
      ...
      v_cto_days := ROUND((v_rec.hours / 8)::numeric, 3);
      ...
      ON CONFLICT (employee_id, year) DO UPDATE SET
        balances = jsonb_set(
          ...
          '{cto}',
          to_jsonb(ROUND((COALESCE((leave_balances.balances->>'cto')::numeric, 0) + v_cto_days)::numeric, 3))
        ),
        updated_at = now();
    END IF;
  END IF;
